<x-layout>
    <div class="max-w-2xl mx-auto bg-base-100 p-6 shadow-lg rounded-xl">
        <h2 class="text-2xl font-bold text-primary mb-4">Form Edit Produk</h2>
        ...
        <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-control mb-4">
                ...
                <input type="text" name="nama_barang"
                    ...
                    placeholder="Contoh: Indomie Ayam Bawang" value="{{ old('nama_barang', $product->nama_barang) }}"
                    required />
                @error('nama_barang')
            ...
            <div class="form-control mb-4">
                ...
                <select require name="jenis_barang"
                    ...
                    <option value="Makanan"
                        {{ old('jenis_barang', $product->jenis_barang) == 'Makanan' ? 'selected' : '' }}>Makanan
                    </option>
                    <option value="Minuman"
                        {{ old('jenis_barang', $product->jenis_barang) == 'Minuman' ? 'selected' : '' }}>Minuman
                    </option>
                    <option value="Bumbu"
                        {{ old('jenis_barang', $product->jenis_barang) == 'Bumbu' ? 'selected' : '' }}>Bumbu</option>
                    <option value="Obat-obatan"
                        {{ old('jenis_barang', $product->jenis_barang) == 'Obat-obatan' ? 'selected' : '' }}>Obat-obatan
                    </option>
                    <option value="Sabun"
                        {{ old('jenis_barang', $product->jenis_barang) == 'Sabun' ? 'selected' : '' }}>Sabun</option>
                    <option value="Lainnya"
                        {{ old('jenis_barang', $product->jenis_barang) == 'Lainnya' ? 'selected' : '' }}>Lainnya
                    </option>
                </select>
                @error('jenis_barang')
                    <span class="text-error text-sm mt-1">{{ $message }}</span>
            ...
            <div class="form-control mb-4">
                ...
                <input type="number" name="harga_pcs" step="0.01"
                    ...
                    value="{{ old('harga_pcs', $product->harga_pcs) }}" />
                @error('harga_pcs')
                    <span class="text-error text-sm mt-1">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-control mb-4">
                ...
                <input type="number" name="harga_2pcs" step="0.01"
                    ...
                    value="{{ old('harga_2pcs', $product->harga_2pcs) }}" />
                @error('harga_2pcs')
                    <span class="text-error text-sm mt-1">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-control mb-4">
                ...
                <input type="url" name="foto_barang"
                    ...
                    value="{{ old('foto_barang', $product->foto_barang) }}" />
                @error('foto_barang')
                    <span class="text-error text-sm mt-1">{{ $message }}</span>
                @enderror
            </div>
